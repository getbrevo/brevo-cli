...
The GA runbook for the two pre-GA features — ordered, mechanical steps for the day each
one ships. It lives on this branch alongside `docs.md` and `QA-TESTCASES.md`, so all three
are tracked independently of the feature branch they were written on.

**Both sections below are durable.** They merge into `main` and stay there until their
feature ships. Do **not** delete either during branch cleanup — each carries its own
final step saying when it may go.

| Section | Lifetime |
| --- | --- |
| `## Before public-apps GA` | Stays until public app distribution ships. |
| `## Before UI-apps GA` | Stays until UI apps (action links) ship. |
...
- [ ] Delete this whole `## Before public-apps GA` section — and if `## Before UI-apps GA`
      has already gone, delete the file and drop the *Working docs* reference to it from
      `CLAUDE.md`.
...
- [ ] Verify nothing was missed:
      ...
      (excluding `node_modules/`, `dist/`, `coverage/`) returns only this file.
- [ ] Delete this whole `## Before UI-apps GA` section — and if `## Before public-apps GA`
      has already gone, delete the file and drop the *Working docs* reference to it from
      `CLAUDE.md`.
