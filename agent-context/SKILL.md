...
Each entry's `context` is optional and per placement: absent means the app receives whatever record context that slot allows, and when present it *narrows* that allow-list to the named fields (it can never widen it). It is per entry because the allow-list belongs to the slot, not the app — a contact page and a deal page can forward different fields. The allowed field names are the registry's to decide, per slot — commonly `recordId`, `recordName`, `userId`, `locale`, `accountId` and `clientId`, but do not treat that as the whole set or assume every slot allows all of them. The CLI does not check names (only that they are non-blank and unique), so an unregistered one is the upload endpoint's 400, not a local error. The context reaches your app as **query parameters** on `redirect_link` — there is no path templating, so read them from the query string. `brevo app create` seeds each entry from the registry's own default for that slot.
