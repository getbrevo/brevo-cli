...
- Do **not** write `link_target`, `extension_point_name` or `sandbox` anywhere in the file — all three are wire/server-stamped values (`app upload` injects `link_target: "_blank"` itself; `sandbox` is the platform's own iframe policy) and the CLI strips them from server echoes.
