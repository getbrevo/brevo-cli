...
- "Change an M2M app's scopes" → `brevo app scopes update --app-id <id> --scopes "<comma-separated>" --mode append --json` (or `--mode replace`). **M2M apps only** — refused for a consent-based OAuth, UI, or Function app. `--mode` is **required** whenever `--scopes` is passed non-interactively; there is no default (in particular no silent default to `replace`, since that can drop scopes). **`append` adds the given scopes and leaves every existing scope alone; `replace` overwrites the whole set** — anything not in `--scopes` under `--mode replace` is removed. Prints a current → new diff and asks to confirm unless `--yes` is passed; a request that changes nothing is a no-op (exit `0`, nothing sent). ASSUMPTION pending the backend API this depends on (BEX-481) — treat the exact wire contract as unverified until that ships. `--app-id` is not optional when scripting, same convention as `app delete`/`app credentials`.
...
- To add, remove, or change scopes on an **M2M app** (no `app-config.json` to edit): run `brevo app scopes update --app-id <id> --scopes "<comma-separated>" --mode append` (to add without touching the rest) or `--mode replace` (to overwrite the whole set) — see the decision-tree entry above. ASSUMPTION pending BEX-481 (the backend API this depends on): treat the exact wire contract as unverified until that ships.
