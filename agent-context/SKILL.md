...
A UI app's `app-config.json` carries a top-level `ui_app` object and exactly `auth: { "type": "none" }` — no scopes, no redirect URLs. Its presence is how the CLI distinguishes the two app types.
