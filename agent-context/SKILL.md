...
- **M2M apps are the exception: their scopes are chosen at creation and cannot be changed afterwards.** `brevo app create --m2m` requires `--scopes "<comma-separated>"`; interactively, the CLI shows a **multi-select over the live IdP catalog** instead of asking for a typed list — the same scopes `brevo app available-scopes` prints, grouped by category with each scope's description, nothing pre-selected, and at least one required. **A whole category can be taken at once by selecting its heading row** (`Account — all 6 scopes`): that ticks every scope in the section, so the grant is on screen and any one of them can be unticked again — dropping one unticks the heading too, since the heading means "all of these". It is a shortcut, not a scope: the app is created with the ticked scope names and never with a category. inquirer's own `<a>` still toggles everything. If the catalog cannot be reached it warns and falls back to the old comma-separated prompt. Because no `app-config.json` is written there is nothing to edit afterwards and no `brevo app upload` to run, so select everything the integration needs the first time.
