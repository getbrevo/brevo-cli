...
- "Change an M2M app's scopes" → `brevo app scopes update --app-id <id> --scopes "<comma-separated>" --json`. **M2M apps only** — refused for a consent-based OAuth, UI, or Function app. **`--scopes` is always the full desired scope set, not a delta** — interactively (omitting `--scopes`), the picker/typed prompt opens pre-filled with the app's *current* scopes so you edit a complete list rather than retype it; non-interactively, whatever you pass in `--scopes` becomes the app's exact new scope list, so include every scope you want kept, not just the ones you're adding. Prints a current → new diff (flagging any scope that would be **removed**) and asks to confirm unless `--yes` is passed; a request that changes nothing is a no-op (exit `0`, nothing sent). ASSUMPTION pending the backend API this depends on (BEX-481) — treat the exact wire contract as unverified until that ships. `--app-id` is not optional when scripting, same convention as `app delete`/`app credentials`.
- "Mint an M2M access token" → `brevo app token --app-id <id> --json`. **M2M apps only** — refused for a consent-based OAuth, UI, or Function app. Prints the Bearer `access_token` and `expires_in`; add `--scope "<comma-separated>"` to request a subset of the app's granted scopes (omit to request the full granted set). The token is short-lived by design and printed in full — it is never written to `~/.brevo/credentials.json`. `--app-id` is not optional when scripting, same convention as `app delete`/`app credentials`. ASSUMPTION pending the backend API this depends on ("brevo app token [Backend]", BEX-482) — treat the exact wire contract as unverified until that ships.
- "Sign out" → `brevo logout --force`

## Hard rules

- **`app-config.json` keys are snake_case (`app_id`, `app_name`, `logo_uri`, `app_type`, `version`, `distribution_type`, `auth.scopes`, `auth.redirect_uris`, `ui_app`, `brevo_function`).** Earlier CLI releases wrote `appId` / `appName` / `logoUri` / `appType` / `auth.redirectUris`. The CLI still reads those, but every write — including a `brevo app upload` or `brevo app scaffold` that has nothing else to change — rewrites the file with the snake_case keys and prints a one-line notice. **If the user's own scripts, CI steps or code read `app-config.json`, tell them to switch to the snake_case names**; do not write the camelCase names into a new or edited file. Commands' `--json` output is a separate contract and is unchanged (`brevo app create --json` still returns `appId`). **`app_type` is a label, not the discriminator** — `brevo app create` / `brevo app scaffold` write it (`"oauth"`, `"ui"` or `"function"`) so the file says in one word what it is, but the presence of the `ui_app` / `brevo_function` / `auth` blocks is what actually decides the type. It is never sent to Brevo — note `brevo app create` *does* send a top-level `app_type` on its API request, but that is a different field with different values (`oauth.consent` / `oauth.m2m` / `ui_app.actionLink` / `brevo_function`), derived by the CLI from the app's own blocks; nothing reads the file's label to build it, and `brevo app upload` sends no `app_type` at all. `brevo app upload` checks the two agree and refuses a config whose label contradicts its blocks, so if you add or remove a `ui_app` block by hand, update `app_type` to match — or delete it, since it is optional and configs written by older releases have no such key.
...
A **UI app** is an action link that renders inside Brevo CRM record pages (contact, company, deal). It is one of three app types (alongside OAuth apps and Brevo Functions; M2M is not a fourth type — it is a *flow* within the OAuth type, see the decision tree) — created by the same `brevo app create` (interactive only, see the decision tree), pushed by the same `brevo app upload`, and made visible in an account with `brevo app install`.
...
- **M2M apps are the exception: they have no `app-config.json` to edit, so scopes at creation are chosen a different way, and changing them later is a different command too.** `brevo app create --m2m` requires `--scopes "<comma-separated>"`; interactively, the CLI shows a **multi-select over the live IdP catalog** instead of asking for a typed list — the same scopes `brevo app available-scopes` prints, grouped by category with each scope's description, nothing pre-selected, and at least one required. **A whole category can be taken at once by selecting its heading row** (`Account — all 6 scopes`): that ticks every scope in the section, so the grant is on screen and any one of them can be unticked again — dropping one unticks the heading too, since the heading means "all of these". It is a shortcut, not a scope: the app is created with the ticked scope names and never with a category. inquirer's own `<a>` still toggles everything. If the catalog cannot be reached it warns and falls back to the old comma-separated prompt. Select everything the integration needs at creation for a smooth start — but see below, this is no longer the only chance: `brevo app scopes update` changes an M2M app's scopes after the fact.
- To add, remove, or change scopes on a **consent-based OAuth app**: edit `auth.scopes` in `app-config.json` directly, then run `brevo app upload`. Comma- or whitespace-separated values are normalized on read, whether they sit in a single array entry (`["crm:read", "crm:write, campaigns:read"]`) or replace the array entirely (`"crm:write, campaigns:read"`) — either becomes two scopes. To see what's currently set, run `brevo app credentials --app-id <id> --json`.
- To add, remove, or change scopes on an **M2M app** (no `app-config.json` to edit): run `brevo app scopes update --app-id <id>` — omitting `--scopes` opens the picker pre-selected with the app's current scopes, so ticking/unticking edits the existing grant rather than starting from nothing; or pass `--scopes "<comma-separated>"` directly as the full new list — see the decision-tree entry above. ASSUMPTION pending BEX-481 (the backend API this depends on): treat the exact wire contract as unverified until that ships.
