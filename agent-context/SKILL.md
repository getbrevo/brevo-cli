...
- `surface_point_list` — one entry per placement. Each entry carries:
  ...
  - `modal_iframe_url` — `iframeExtension` entries only; rejected on an `actionLink`. The page Brevo embeds in the modal; record context arrives as query parameters, same as `redirect_link`.
