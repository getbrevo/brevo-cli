...
- "Submit a public app for review" → `brevo app submit --app-id <id> --json` (prints the submission form URL as `{"app_id","form_url"}` without opening a browser; without `--json` it opens the form in the user's browser). The app's `distribution_type` must be `public`, and when `app-config.json` describes the target app it must match the server — push local changes with `brevo app upload` first if the command reports drift.
