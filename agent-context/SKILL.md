...
Some Brevo features are not released yet, and the CLI **hides the commands and flags that drive them** rather than offering something that can't work. What `brevo --help` and `brevo app --help` list is what is available to the account you're logged in as — treat that as authoritative and build from it.

Two consequences:

- **Don't suggest, script, or retry a command that isn't listed.** Invoking a hidden one exits `1` with *"That command is not available yet. It is part of a Brevo feature that has not been released."* That is a final answer, not a transient failure — there is no flag, config edit, or alternative route around it. Tell the user the feature isn't released and offer the nearest thing that works.
- **The list can differ between accounts**, so read it in the current session rather than relying on what you saw elsewhere or on this document. Accounts enrolled in a preview see more; the extra commands are documented in *Commands you may not see* below, so they're covered if you have them.

This is a convenience guard, not a permission system. The Brevo API is the real authority and refuses unreleased features independently — so a command that slips through still fails server-side, with the platform's own message. Report that message rather than working around it.
...
## Commands you may not see

The commands below drive features that are still in preview, so **most accounts don't have them** — they're absent from `brevo app --help` and refused if invoked. Check the help output before reaching for any of them; if one isn't listed, it isn't available and the answer to the user is that the feature isn't released yet.

If your `brevo app --help` *does* list them, they work normally and this is their reference.

**Public-app review lifecycle** — these apply only to an app whose `distribution_type` is `public`:

- "Check an app's review status" → `brevo app status --app-id <id> --json` (read-only; returns `{ state, message }`, `state` ∈ `configured`/`submitted`/`in_review`/`approved`/`rejected`/`changes_requested`, or `unknown` when the server returns no state. Reviewer feedback comes by email, not here.)
