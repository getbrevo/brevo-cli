...
- "Create a UI app" (an extension that renders inside Brevo CRM records — an action link, or an iframe opened in a modal) → run `brevo app create` **interactively** and pick *UI app* at the *"What type of app are you building?"* prompt, **or non-interactively** with `--ui-app --record-page <slug> --placement <surface_point_name> --label <text> --url <url> [--more-info <text>]`, or `--ui-config <file>` (a JSON file: `{ extension_type, record_page, surface_point_name, label, more_info?, redirect_link }`). There is still **no `--type` flag** — a plain non-interactive run with neither `--ui-config` nor `--ui-app` creates an OAuth app, exactly as before. Both non-interactive routes only support `extension_type: "actionLink"` today; an unknown `--record-page`/`--placement` fails with the valid options listed in the error, and a `--ui-config` file carrying a key it does not read is **refused by name** rather than silently dropped — `iframe_href`, `layout`, `modal_size` and `modal_height` because they are `iframeExtension`-only (create the app interactively, or add them to `app-config.json` and `brevo app upload`), and `link_target`, `sandbox`, `extension_point_name` and `version` because the platform owns those and stamps them itself (remove them; they are not authored in `app-config.json` either). Interactively, the flow asks for the integration type (*Link* opens your URL in a new tab; *Iframe* embeds your page in a modal, offered **only on private apps** — iframe extensions are private-only, and both non-interactive routes stay `actionLink`-only), one record page, one placement on it, a label, optional supporting text, and the destination URL (`redirect_link` for a Link, `iframe_href` for an Iframe), then — for an Iframe only — how it presents (`layout`, on a widget placement) and how big the modal is (`modal_size`, whenever a modal actually opens); both default answers write nothing — every route authors exactly **one** placement; add more by hand as further `surface_point_list` entries in `app-config.json` (each with its own `label` and destination URL), then `brevo app upload`.
...
- `surface_point_list` — one entry per placement. Each entry carries:
  ...
  - `redirect_link` — `actionLink` entries only: the destination URL that entry opens; record context arrives as **query parameters** (the path is never templated). Rejected on an `iframeExtension` entry.
  ...
  - `iframe_href` — `iframeExtension` entries only; rejected on an `actionLink`. The page Brevo embeds — in the modal, or directly in the card body when the entry's `layout` is `"inline"`; record context arrives as query parameters, same as `redirect_link`.
  - `layout` — `iframeExtension` entries on **widget slots** only: `"inline"` embeds the page directly in the card body, `"modal"` (or omitting the key — the default) opens it from the card's CTA button. `brevo app create` asks this for an iframe widget placement and writes the key only for `inline`. Rejected on an `actionLink` entry, and `"inline"` is rejected on a slot that renders no card.
  - `modal_size` — `iframeExtension` entries that open a modal: `"small"`, `"medium"`, or `"large"` (the default — omit the key for it). Gated **differently from `layout`**: a `.action` slot's menu entry always opens a modal (and takes no `layout` at all), a widget slot opens one unless its `layout` is `"inline"`. `brevo app create` asks it whenever a modal opens and writes the key only for a non-default answer. Rejected on an `actionLink` entry.
  - `modal_height` — same gating as `modal_size`, but a CSS length instead of a preset: `"<positive integer>px"` (unbounded — the dialog clips), `"<1-100>vh"` or `"<1-100>%"` (both viewport-relative, hence the bound). Absent means the dialog kit's own default height applies. Hand-edit only — `brevo app create` asks no question for it, same treatment as the widget card's `size`. Rejected on an `actionLink` entry.
- Do **not** write `link_target`, `extension_point_name` or `sandbox` anywhere in the file — all three are wire/server-stamped values (`app upload` injects `link_target: "_blank"` itself; `sandbox` is the platform's own iframe policy) and the CLI strips them from server echoes. An authored `sandbox` is refused by name, at the `ui_app` root and on any entry, before `app upload` makes a single call: what an embedded iframe may do is Brevo's decision, and the UI kit applies the stored value verbatim with no default of its own — an entry served without one renders an inert frame.
...
- **Iframe extensions are private-only**: `extension_type: "iframeExtension"` requires `distribution_type: "private"`. The create prompt only offers *Iframe* on a private app, `brevo app upload` refuses the combination locally before any round trip, and the platform 400s it — same rule, three places.
- **Iframe extensions also roll out per account**, separately from the private-only rule above. The prompt cannot know this — it still offers *Iframe* on every private app — so a `400` naming the account's iframe access is possible even on a private app; both `create` and `upload` turn it into a message suggesting *Link* or asking Brevo to enable it.
