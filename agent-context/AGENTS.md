...
| `brevo app status` | Show an app's review status (`--app-id`, `--json`). Read-only. `--json` returns `{ state, message }` where `state` is one of `configured`, `submitted`, `in_review`, `approved`, `rejected`, `changes_requested`, or `unknown` when the server returns no state; `message` is canned copy. Reviewer feedback is sent by email, never in this output. |
