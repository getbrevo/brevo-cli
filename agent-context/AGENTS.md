...
| `brevo app rollback <account-id>` | **⚠️ Not available yet — UI apps only.** Roll back an app from one Brevo account (`--app-id`, `--force`, `--json`). No upload gate. If the app isn't deployed there it says so and exits `0` — `{"rolledBack": false, "reason": "NOT_DEPLOYED"}` under `--json` — so teardown stays idempotent. |
