...
- **Public apps are not available yet.** Always create apps with `--distribution private`, never set `distribution_type` to `public` in `app-config.json`, and don't run `brevo app submit` / `brevo app status` / `brevo app withdraw` — unless the user is deliberately testing the public-app flow. See the notice at the top of this file, including its *Exception — deliberate testing* clause.
