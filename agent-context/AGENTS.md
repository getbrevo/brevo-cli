...
**The API now enforces this for `create`.** `brevo app create --distribution public` fails with *"Brevo refused to create a public app from the CLI — public app distribution is not available yet"* (the platform answers `400 invalid_parameter`, keyed on the CLI being the caller). It cannot be worked around from the CLI — not with flags, not by editing `app-config.json`. Don't retry it or look for another route; switch to `--distribution private`.
...
- **Ends in `@brevo.com` or `@sendinblue.com`** → this is a Brevo internal account, i.e. someone building or testing the CLI itself. The rule above does **not** apply: run the review-lifecycle commands and help normally. Say once that public apps aren't live for end users so they know what they're pointing at, then stop warning and don't refuse anything. **One limit is not yours to lift:** the API refuses a public-app `create` from the CLI for *every* caller — it keys on the CLI, not on the account — so `--distribution public` fails for internal accounts too. Report that as the platform's answer rather than retrying it.
