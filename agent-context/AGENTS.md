...
| `brevo app list` | List apps — OAuth and UI (`--json`). Each row names its type; a UI app has no client ID, callbacks or scopes, so those rows are omitted for it. |
