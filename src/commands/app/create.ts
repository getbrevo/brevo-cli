...
async function resolveAppType(interactive: boolean, distribution?: string): Promise<AppType> {
  ...
  // Brevo Functions are a private-distribution-only app type. The condition is a
  // product constraint, not a readiness gate — Functions ship in every build, so the
  // `isFeatureAvailable` check this site carried pre-GA went with the gate itself.
