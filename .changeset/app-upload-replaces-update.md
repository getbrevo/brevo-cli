...
UI apps no longer carry an OAuth block (BEX-290): a UI app's `app-config.json` now has exactly `auth: { "type": "none" }` — no `scopes`, no `redirectUris` — and `brevo app create` no longer applies the narrower UI-app scope defaults (the OAuth defaults are unchanged). On the wire, UI-app creates omit the `scopes` key and UI-app uploads omit the whole `auth` key. `brevo app upload` enforces the shape both ways: a UI-app config still carrying `scopes`/`redirectUris` is rejected with instructions to switch to `{ "type": "none" }`, and `"type": "none"` on a config without a `ui_app` block is rejected too.
