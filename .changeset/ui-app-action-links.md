...
Stop a failed read-back from destroying a successful `brevo app create`.

After creating an app, `brevo app create` reads it back to build the scaffold. When that read answered `404 id not found` — observed on staging for a UI app, for an ID the create endpoint had issued under a second earlier — the error propagated before any file was written: the command exited non-zero with `App <id> not found.` while the app sat on the server, and re-running produced another one. The create response is now passed in as a fallback, so a read that comes back empty or 404 falls back to it (it carries the name, distribution type, logo, version and credentials the scaffold needs) and the run completes with a warning naming the app and pointing at `brevo app scaffold` to refresh the config later. Under `--json` the warning is suppressed, since it would otherwise land in the middle of the JSON blob.

This is scoped to that one read. A 404 anywhere else — `brevo app scaffold`, `app upload`, `app delete`, `app status` — still means the app ID is wrong and still fails, and even on the create read-back only 404 is tolerated: a 500 or an expired session surfaces as before rather than being quietly scaffolded around. Whether the platform should resolve that ID at all is a separate question, tracked in `RELEASE-CHECKLIST.md`; the fallback is correct either way and stops applying by itself once the read succeeds.

Name each placement's slot key `surface_point_name` (BEX-290, eighth round). This supersedes every `surface_point` in the example blocks and migration hints above — where they disagree, this one is current.

The value never changed: an entry always named its slot with the registry's kebab-case `surface_point_name` slug, and the platform always resolved it with `WHERE surface_point_name = ANY(...)`. The KEY said `surface_point`, which is also the name of a different field on the registry row — the *dotted* `extension_point_name`. So the one identity a partner must not confuse was carried under a key that named the other one, and the previous round's bug (authoring the dotted name, then failing the first upload with `contains unregistered extension point(s)`) was that confusion made concrete. The key now names the column it is matched against, so the entry field, the registry column and the `WHERE` clause all read the same word.

The name `surface_point` is now used nowhere — not as an authored key, not as a rejection hint, and not on the registry response, whose row field was the last holdout and is now `extension_point_name`, the column it always read. There is no alias and no migration hint: an entry still carrying the old key fails the generic unknown-key check. UI apps are not live, so the only configs affected are on developer machines.
