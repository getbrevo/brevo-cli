...
**Wire contracts — what is confirmed and what is still assumed.** The upload contract is confirmed against the platform's CLI upload endpoint: the block travels under the `ui_app` key on `POST /v3/app-store/apps/{id}/upload` (the earlier `snapshot` key was rejected server-side; "snapshot" on the platform names the whole stored app config). The deploy transport is confirmed too, and it is **one resource, not two routes** — `POST /v3/app-store/apps/{id}/installs` to install and `DELETE` on the same path to remove, both carrying `deploy_client_id` (the account ID, **as a number**), `name`, and `is_developer`. The `deploy` / `rollback` commands are named for the partner-facing verb; the resource is an install. What remains assumed is only HTTP 422 for "not uploaded" / "not deployed", and whether the POST response carries an install ID worth surfacing — both marked in code comments and tracked in `RELEASE-CHECKLIST.md` → *Before UI-apps GA*.
