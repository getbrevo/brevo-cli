...
UI apps (action links that render inside Brevo CRM records) are **not live on the platform**. The repo contains the surface — the *UI app* choice at `brevo app create`'s app-type prompt, `brevo app install [account-id]`, `brevo app uninstall [account-id]` — and, exactly as with public apps, a **published build does not**: the two commands are eliminated from the bundle and the app-type prompt is not asked, so a public build creates an OAuth app exactly as it did before BEX-290.
