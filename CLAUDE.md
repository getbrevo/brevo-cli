...
- **`docs.md` — the open-questions log.** Everything still open on public apps after GA:
  the release-gate coverage gap, TC-6.3's remaining half, the BEX-355 / BEX-350 / BEX-437
  sign-offs, and the QA gaps. It used to have a *Part 1* holding release copy until GA;
  that copy was consumed into the changeset and the split is gone. The gate-machinery
  teardown was tracked here too and is now done.
- **`QA-TESTCASES.md` — the manual public-apps test plan** (suites 2, 5, 6, 7, 10, 13),
  with the recorded sweep results. The recorded results were taken on the pre-GA
  `PREVIEW=1` artifacts, which no longer exist — there is one build now; re-baselining
  them is tracked in `docs.md`.
