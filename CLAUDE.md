...
- **`iframe_href` is gated on `extension_type`.** The UI kit keeps it only for `iframeExtension`, so the CLI rejects it on an `actionLink` rather than letting a partner ship a URL that never opens. **`iframeExtension` is pre-GA (BEX-459) and gated at build time**: `FEATURE_STAGE['ui-iframe-type']` is `'preview'`, the *Iframe* choice sits behind `__BREVO_PREVIEW__ && isFeatureAvailable('ui-iframe-type')` in `promptIntegrationType` (an ELIMINATION SITE — its label lives in `src/lang/preview-messages.ts` so a published bundle carries neither), and `uiAppType.validateConfig` calls `assertFeatureAvailable('ui-iframe-type')` ahead of every shape check so a hand-edited `app-config.json` is refused before any round trip. It is gated because the platform is not ready for it on either side: the extension-point registry has no slot enabled for `iframeExtension`, and bo-be's per-account Unleash flag is off by default — so an ungated choice would dead-end for every partner in a way that reads as a CLI bug. The rest of the iframe contract below is live code and stays accurate; it simply is not reachable in a published build. In a `PREVIEW=1` build the *Iframe* choice appears on a **private** app only, because **iframe extensions are private-only (v1)**: the same rule is enforced by `uiAppType.validateConfig` locally at upload (`APP_UI_IFRAME_PRIVATE_ONLY`) and by bo-be at upload/create (`validateIframeDistribution`), so all three layers refuse the combination with the same wording. The prompt gate reads the distribution answer `create` already collected (`resolveUiApp(distribution)`); the iframe branch writes the URL answer to `iframe_href` instead of `redirect_link` and everything downstream (diff, write-back, wire-only strip) already handled both fields. **It was `modal_iframe_url` until BEX-467** — a misnomer once `layout: "inline"` existed, because an inline card's iframe opens no modal. The rename went through all four repos in the chain at once (`iframeHref` in the manifest and the UI kit, `iframe_href` here and on the bo-be wire), and it needs **no migration and no alias**: the registry flip that would let an `iframeExtension` app exist has not been applied on staging or production, so no `app-config.json` in the wild can carry the old key. Do **not** add it to the `LEGACY_KEY_ALIASES` tables in `src/lib/config.ts` — those are for the BEX-470 camelCase→snake_case migration of files that really do exist. The upload diff's row is `iframe URL:`, named for the field rather than the presentation, for the same reason the field is. **Separately, iframe-extension authoring is behind a per-account Unleash flag on bo-be** (`app-store-bo-be-iframe-extension`, checked by the same `validateIframeDistribution` before the private-only rule). Unlike private-only, this one is NOT enforced locally — the flag is per-account, so a local copy could only lag it, same reasoning as `CLAUDE.md`'s standing rule against mirroring per-account platform policy. In a preview build the prompt still offers Iframe on every private app; a client without the flag only finds out from the server's `400`, which `isIframeExtensionDisabledRefusal` in both `app create` and `app upload` translates into a message naming Link or asking Brevo to enable it (`APP_CREATE_UI_IFRAME_DISABLED` / `APP_UPLOAD_UI_IFRAME_DISABLED`), narrowed on the flag's name so an unrelated 400 keeps the server's own text.
...
- **The strip is inbound-only, so `sandbox` needs a refusal as well.** `stripUiAppWireOnlyKeys` runs on what the server *echoes*; `app upload` sends the file's `ui_app` block verbatim (plus the injected `link_target`), so a key authored into `app-config.json` travels outbound unexamined. That is tolerable for the other three — `version` and `extension_point_name` are inert, `link_target` is pinned per entry — but not for `sandbox`, which is the iframe's own permission set: bo-be stamps it at write time, app-store-backend serves the stored value on the manifest, and the UI kit applies it verbatim with **no default of its own** (absent ⇒ the fail-closed `sandbox=""`). So `validateUiApp` refuses an authored one by name at the root and on every entry (`rejectAuthoredSandbox`), ahead of the per-entry checks, and `--ui-config` refuses it a layer earlier off the same `wireOnlyKeys` list. Refused rather than stripped on the way out, deliberately: silently dropping a security-relevant key leaves the partner believing their value is in force.
