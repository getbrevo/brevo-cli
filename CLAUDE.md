...
**Release gates: `scripts/release-check.mjs`, two modes, one set of assertions.** `release.yaml` runs `pre` before the publish (on the tarball `npm pack` produces) and `post` after it (on the tarball the registry serves), so the gate before the publish and the gate after it are checking the same thing about the same artifact. Run either locally: `yarn release:check pre`, `yarn release:check post --version=2.2.0`.

The shared half asserts the tarball's shape: `REQUIRED_FILES` are present (`files:` is an allow-list, so a loss is silent — a missing `agent-context/SKILL.md` ships a CLI whose skill doesn't exist), `FORBIDDEN_PATTERNS` are absent (the branch-local working docs above, plus `.env` / `credentials.json` / `.brevo.json` / keys), every `src/templates/files/*.tmpl` made it into `dist/` (counted against the source dir, so a twelfth template needs no edit here), the packed content holds no secret-shaped string, and **the tarball installs into an empty tree and the installed `brevo --version` runs**. That last one is the only check on the dependency closure: esbuild keeps `commander`/`inquirer` external, so a dep that drifted into `devDependencies` builds, tests and packs fine and then fails on the first user's machine with `MODULE_NOT_FOUND`. Running the bundle from this repo would resolve against the repo's own `node_modules` and always pass — hence the install.

`post` adds the registry metadata that only exists after a publish: the version resolves (retried for 5 minutes of propagation), `latest` moved to it, a SLSA provenance attestation is attached, the publisher is the OIDC identity (`npm-oidc-no-reply@github.com` — a token publish is not), and the download matches the advertised `dist.integrity`. Those last two are the OIDC posture below, asserted rather than assumed; if they fail, fix the trusted publisher on npmjs.com — **do not** add an `NPM_TOKEN`.

Two things it deliberately does not do. It **does not re-check the gated public-app surface** — `scripts/build.mjs` owns `LEAK_MARKERS` / `LEAK_STRINGS` / `GA_MARKERS`, throws at build time, and `prepublishOnly` rebuilds so it runs again on the publish itself; a second copy of that list here could only drift. And **`post` is not the smoke test** — smoke authenticates and drives real commands (`smoke.yml`), this only proves the artifact and its provenance. `.tmpl` paths are skipped in the forbidden-*filename* scan (the template set includes `.env.example.tmpl` and `app-config.json.tmpl` on purpose); their content is still secret-scanned.
