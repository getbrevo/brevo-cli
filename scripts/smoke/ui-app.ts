...
function createExchanges(integration: UiIntegration): PtyExchange[] {
  return [
    ...
    integration === 'iframe'
      ? {
          // Waits on the LINK choice, not the question: Link is always offered, so its
          // line proves the whole choice list has rendered before the transcript is
          // probed for Iframe. Matching the question instead would let a chunk carrying
          // only the question satisfy the expect, find no Iframe yet, and skip the whole
          // leg as "this build has no Iframe choice" — a false green. Same shape as the
          // app-type exchange above (`appTypeOAuth` gates, `appTypeUi` probes).
          expect: UI_CREATE_EXPECT.integrationLink,
